<template>
  <div class="flex h-screen bg-transparent font-sans text-slate-800">
    ...
    <div
      v-if="showFormModal"
      class="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-fade-in"
    >
      <div
...
const { bugsList, isLoading, fetchBugs, addBug, updateBug, deleteBug } =
  useBugs("หน้าแดชบอร์ด");

// SweetAlert Toast (ยังใช้ Swal สำหรับการแจ้งเตือนมุมจออยู่เพราะสวย)
let Toast = null;
onMounted(() => {
  fetchBugs();
...
// --- ฟังก์ชันสำหรับ Native Loading ของเราเอง ---
...
// รายการบั๊กที่ผ่านการ Filter และค้นหา
...
const itemsPerPage = ref(10); // จำนวนที่ต้องการแสดงต่อ 1 หน้า

// หากมีการค้นหาหรือเปลี่ยนฟิลเตอร์ ให้กลับไปที่หน้าแรกเสมอ
...
// ตัดแบ่งข้อมูลมาแสดงเฉพาะหน้าปัจจุบัน
...
// คำนวณเลขหน้าที่จะแสดง (แสดงแบบมี ... ถ้ารายการหน้าเยอะเกิน)
const visiblePages = computed(() => {
  const total = totalPages.value;
  const current = currentPage.value;
  const delta = 1; // จำนวนหน้าที่ต้องการแสดงข้างๆ หน้าปัจจุบัน
...
const saveBug = async () => {
  ...
  // เรียกโชว์ Loading Popup ของเราเอง
  showLoadingPopup(
    formType.value === "add" ? "กำลังบันทึกข้อมูล..." : "กำลังอัปเดตข้อมูล...",
  );
  ...
  if (bugToDelete.value) {
    ...
    try {
      ...
      if (success) {
        ...
        // กรณีถ้าลบตัวสุดท้ายของหน้า ให้ถอยกลับมา 1 หน้า
