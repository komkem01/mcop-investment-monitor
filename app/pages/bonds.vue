          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="space-y-1.5">
              ...
              <input
                ...
                placeholder="ตัวอย่าง ปรับให้ตรงตาม Figma การแสดงผลตารางรายงาน"
    ...
    >
      ...
      >
        ...
        <p class="text-xs text-slate-500 font-bold leading-relaxed mb-6">
          ...
          (${bugToDelete?.bugId ? bugToDelete.bugId : ''}) <br />
...
const visiblePages = computed(() => {
  const total = totalPages.value, current = currentPage.value, delta = 1;
...
const formState = ref({
  id: null, sheetRowIndex: null, bugId: "", title: "", comments: [],
  expectation: "", driveLink: "", figmaLink: "", status: "Defect",
  createdDate: "", fixedDate: "", passedDate: "",
});
...
const openAddModal = () => {
  ...
  formState.value = {
    id: null, sheetRowIndex: null, bugId: generateNextBugId(), title: "", comments: [],
    expectation: "", driveLink: "", figmaLink: "", status: "Defect",
    createdDate: new Date().toLocaleString("th-TH"), fixedDate: "", passedDate: "",
  };
...
const saveBug = async () => {
  ...
  if (formType.value === "add") {
    ...
    const success = await addBug({
      bugId: formState.value.bugId, title: formState.value.title, comments: formState.value.comments,
      expectation: formState.value.expectation, driveLink: formState.value.driveLink,
      figmaLink: formState.value.figmaLink, status: formState.value.status,
      createdDate: formState.value.createdDate, fixedDate: formState.value.fixedDate, passedDate: formState.value.passedDate,
    });
    closeLoadingPopup();
  ...
  if (bugToDelete.value) {
    ...
    try {
      ...
      if (success) {
        ...
        if (paginatedBugs.value.length === 0 && currentPage.value > 1) currentPage.value--;
      } else {
